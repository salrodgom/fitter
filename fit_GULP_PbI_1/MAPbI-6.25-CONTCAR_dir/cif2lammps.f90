program perovskites_CIFFiles2Lammps
 use iso_fortran_env
 implicit none
! locals
...
! Zn...OX, Zn...HX, Zn...CX
 do i=1,n_atoms
  do j=1,n_atoms
   if(i/=j.and.ConnectedAtoms(i,j))then
    ...
    else if ( ( atom(i)%element==82.or.atom(j)%element==82 ).or.&
              ( atom(i)%element==53.or.atom(j)%element==53 ).or.&
              ( atom(i)%element==35.or.atom(j)%element==35 )) then
     ConnectedAtoms(i,j)=.false.
     ConnectedAtoms(j,i)=.false.
     atom(i)%degree=atom(i)%degree-1
     atom(j)%degree=atom(j)%degree-1
 ...
 call check_bonds_with_degree(53,1)
 call check_bonds_with_degree(83,1)
! degree of each node:
 ...
 if ( modify_topology_flag ) then
  ...
  scan_for_rename_first_phase: do i=1,n_atoms
   ...
   if( atom(i)%element==7 )then
    atom(i)%new_label='N1  '
    atom(i)%charge = -1.1
   else if( atom(i)%element==82) then
    atom(i)%new_label='Pb  '
    atom(i)%charge = +2.03
   else if( atom(i)%element==35) then
    atom(i)%new_label='Br  '
    atom(i)%charge = -1.13
   else if( atom(i)%element==6) then
    atom(i)%new_label='C1  '
    atom(i)%charge = +0.771
   else if( atom(i)%element==53) then
    atom(i)%new_label='I  '
    atom(i)%charge = -1.13
   end if
   ...
   if ( atom(i)%element == 1) then
    do j=1,n_atoms
     if(j/=i.and.ConnectedAtoms(i,j))then
      if( atom(j)%new_label == "C1  " )      then
       atom(i)%new_label = "H1  "
       atom(i)%charge = +0.023
      else if( atom(j)%new_label == "N1  " ) then
       atom(i)%new_label = "H2  "
       atom(i)%charge = +0.54
      else
       write(6,*)'============================================================'
    ...
    do
     ...
     if(line(1:3)=="End") then
      ...
      if((adjustl(trim(ourlabel(1)))==adjustl(trim(label(1))).and.&
         ...
         (adjustl(trim(ourlabel(1)))==adjustl(trim(label(2))).and.&
          adjustl(trim(ourlabel(2)))==adjustl(trim(label(1)))))then
       !write(6,'(4(a4,1x))')label(1),label(2),ourlabel(1),ourlabel(2)
       !write(6,'(a)')passing
       !read(passing,'(a,a)')type_fff,variables
       !if(adjustl(trim(passing))/='none')then
       ! read(variables,*) p1,p2
       ! !write(6,*)p1,p2
       ! select case(units)
       !  case('kcal/mol')
...
end program perovskites_CIFFiles2Lammps
